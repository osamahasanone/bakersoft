...
router = {
    ...
    "work_tracking.tasks.notify_team_leaders_about_tasks_overdue_soon": purge_queue,  # Noqa
}
...
def task_router(name, args, kwargs, options, task=None, **kw):
    try:
        ...
        logger.error(
            "Task %s has not been routed! Please add it to the `config.celery.task_router`.",  # Noqa
            name,
        )
